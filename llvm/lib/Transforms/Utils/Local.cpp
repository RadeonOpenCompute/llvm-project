...
void llvm::ConvertDebugDeclareToDebugValue(DPValue *DPV, LoadInst *LI,
  ...
  if (!DIExpr->holdsNewElements() &&
      !valueCoversEntireFragment(LI->getType(), DPV)) {
    // FIXME: If only referring to a part of the variable described by the
...
void llvm::ConvertDebugDeclareToDebugValue(DPValue *DPV, PHINode *APN,
  ...
  if (!DIExpr->holdsNewElements() &&
      !valueCoversEntireFragment(APN->getType(), DPV)) {
    // FIXME: If only referring to a part of the variable described by the
    // dbg.declare, then we want to insert a DPValue for the corresponding
    // fragment.
    LLVM_DEBUG(dbgs() << "Failed to convert dbg.declare to DPValue: " << *DPV
        ...
        if (StoreInst *SI = dyn_cast<StoreInst>(U)) {
          ...
          if (!CI->isLifetimeStartOrEnd()) {
            ...
            if (DDI->getExpression()->holdsNewElements()) {
              ...
              insertDbgValueOrDPValue(DIB, AI, DDI->getVariable(),
...
bool llvm::replaceAllDbgUsesWith(Instruction &From, Value &To,
  ...
  auto IdentityDPV = [&](DPValue &DPV) -> DbgValReplacement {
    if (DPV.getExpression()->holdsNewElements())
      return updateNewDIExpressionArgType(DPV, &From, ToTy);
